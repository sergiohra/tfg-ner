...
filter_nes_pos.py
-----------------
Filtra tokens que claramente NO son Entidades Nombradas (NEs) de un fichero
de términos con frecuencias en formato:

    término: frecuencia

Criterio de filtrado — basado en reglas estructurales/lingüísticas objetivas,
sin listas hardcodeadas dependientes del dominio:

  REGLA 1 — Numérico puro:
      El término está compuesto únicamente por dígitos y separadores numéricos.

  REGLA 2 — URL o email:
      El término contiene una URL (http/www) o dirección de correo.

  REGLA 3 — Fragmento partido:
      El término empieza o termina con comillas sin cerrar.

  REGLA 4 — POS tagging con spaCy (es_core_news_lg):
      Se analiza morfosintácticamente el término:

      Término de UN SOLO TOKEN:
        - Si spaCy lo etiqueta como PROPN (nombre propio) o X (sigla/extranjero)
          → CONSERVAR
        - Si lo etiqueta como NOUN, VERB, ADV, ADP, DET, etc.
          → ELIMINAR
        El razonamiento es que un nombre propio aislado puede ser NE;
        un sustantivo común aislado no lo es.

      Término MULTITOKEN:
        - Si algún token es PROPN o X → CONSERVAR (contiene nombre propio)
        - Si la mayoría de tokens son VERB/ADV/ADP/DET/PRON (no nominales)
          → ELIMINAR
        - Si la frase es puramente nominal (NOUN/ADJ) → CONSERVAR
          (pueden ser entidades de dominio compuestas como "seguridad social")

Uso:
    python filter_nes_pos.py [--input FILE] [--output FILE] [--rejected FILE] [--model MODEL]
