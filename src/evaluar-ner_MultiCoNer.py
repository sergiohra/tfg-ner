...
TRAZA_FILE = "traza_multiconer.txt" # Archivo de traza 
RESULTADOS_FILE = "resultados_multiconer.txt"# Archivo de resultados
N = 5000 # Número de frases a evaluar (ajustable)
NUM_EJEMPLOS_TRAZA = 20 # Número de ejemplos detallados en la traza (predicciones + entidades detectadas)
BATCH_SIZE_HF = 32 # Tamaño de lote para Hugging Face
BATCH_SIZE_SPACY = 500 # Tamaño de lote para spaCy
...
# Función de logging para traza y resultados
def log(msg, fh):
    print(msg)
    fh.write(msg + "\n")

#Mapeo de etiquetas fine-grained a coarse-grained (PER, LOC, ORG)
FINE_TO_COARSE = {
...
# Solo consideraremos estas 3 categorías para que la evaluación sea justa.
KEEP_TYPES = {"PER", "LOC", "ORG"}

# Función para mapear etiquetas fine-grained a coarse-grained, o "O" si no es relevante
def map_tag(tag):
    if tag == "O":
        return "O"
...
log(f"\n▶ [1/3] HuggingFace — batch_size={BATCH_SIZE_HF}...", ft)
t0 = time.time()
...
for idx in range(min(NUM_EJEMPLOS_TRAZA, N)):
    log(f"\n  [HF] Frase {idx}: {sentences_text[idx][:80]}{'...' if len(sentences_text[idx]) > 80 else ''}", ft)
...
# ─────────────────────────────────────────────────────────────
# 8. EVALUACIÓN CON NERVALUATE → ARCHIVO DE RESULTADOS
# ─────────────────────────────────────────────────────────────
